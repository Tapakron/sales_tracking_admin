...
class SalesController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        $uploadImg = GlobalFunc::uploadImg($request, $user, $body['user_id'], 'user_img', 'user'); //! request จากหน้าบ้าน,ข้อมูล user, id ไปใส่ชื่อไฟล์ , ตัวแปร , ชื่อไฟล์จะเก็บรูป
        if ($body['avatar_remove'] == '1' && $uploadImg == null) {
            $body['user_img'] = null;
        } else {
            if ($uploadImg != null) {
                $body['user_img'] = array_key_exists('user_img', $uploadImg) ? $uploadImg["user_img"] : NULL;
